...
| Purple's cursor cell uses `accent-deep`, not `accent` | Mark's call, by eye: the vivid `#A100FF` was too bright as a solid block at cell size. It also raises white-on-fill from 5.30:1 to 8.34:1. Vivid P still carries the Check button and the SOLVED stamp — smaller or momentary blocks. Green is unaffected (it keeps `accent` for the cursor). |
