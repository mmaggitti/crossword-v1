...
| Three roles carry the accent's derived uses: `--accent-ink`, `--on-accent-quiet`, `--accent-fill` | The spec's role set is otherwise closed, so this is a deliberate addition. Vivid purple `#A100FF` on its own `#E6DCFF` tint is **4.05:1** — under AA in *both* directions, so it fails as the clue pill's text and as the cell number on an accent fill. Rather than fork those two rules per theme, the roles absorb it: green aliases them to the existing tokens (byte-identical rendering), purple points them at `accent-deep` (6.37:1) and `surface` (5.30:1). Gated by `theme-test.cjs`. |
| Purple fills all use `accent-deep`, not `accent` (`--accent-fill`) | Mark's call, by eye: the vivid `#A100FF` was too bright as a large solid block. Every solid fill — cursor cell, buttons, SOLVED stamp, update prompt — routes through `--accent-fill`, which purple points at `accent-deep`; it also raises white-on-fill from 5.30:1 to 8.34:1 throughout. Vivid P is left to the solved frame and the SOLVED wordmark (a stroke and letterspaced text, not area). Green is unaffected — it aliases `--accent-fill` to `accent`. |
